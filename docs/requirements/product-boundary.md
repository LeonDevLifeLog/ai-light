...
| 层 | 名称 | 职责 | 状态 | 关键依据 |
|---|---|---|---|---|
| L1 | **接入层** | 接收各智能体客户端 hook 事件 | ✅ CLI Adapter 已实现，实机闭环待验收 | `docs/specs/adapter-cli.md` + `docs/specs/hook-api.md`；支持 Claude Code / Codex / Qoder / TraeCode / WorkBuddy |
...
- TraeCode——CLI 安全合并 `~/.trae-cn/hooks.json` schema v1 全局 command hooks；不依赖 Claude Hook 导入
- WorkBuddy——CLI 安全合并 `~/.workbuddy/settings.json`；仅使用官方明确支持的事件

**暂缓/不覆盖**：Cursor（🟡 桥接方案存档）、Claude Desktop 纯聊天（🔴 不覆盖，内置 Claude Code 可接）。

**待办**：发布含 Qoder 与 TraeCode 支持的 `@ai-light/adapter`、修复本机 Codex 安装、完成各工具与 AgentCore-Light 真机闭环验收。
...
- 产品形态：托盘常驻 + 启动即显示主窗口 + 关窗 = 隐藏（托盘唤回）+ 退出只经托盘（✅ 已实装 2026-08-21，KAD-06；macOS 用 Accessory 策略，Dock 不显示图标）。旧 Q2 表述"窗口只在配置/试听时出现"已作废。
- 内容：设备管理、主题编辑、状态展示、手动灯效试听/测试
- UI 设计排期：已启动并实装（5 页 + 托盘 + 设置页，见 ui-design.md）
