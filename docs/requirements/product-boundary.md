...
1. L1 hook 协议形态：HTTP POST vs 其他（待调研后定）
2. L1 事件模型：开放状态名 + 标准状态名集（待调研后定）
3. L2 仲裁规则：优先级抢占 vs 最近活跃（默认优先级抢占，可配置）
4. 会话支持：单会话为主、多会话可配置（对应"单灯/多灯"）
5. 安全边界：仅回环 127.0.0.1 + 可选 token
6. 高级直控通道（直接传 SCENE）是否预留：建议预留枚举、V2 实现
