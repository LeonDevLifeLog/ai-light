...
- 用户主题：导入 `themes/` 目录（app config dir），优先级高于内置同名主题
...
4. **静音段 volume 语义**：theme-format V1.0 要求 volume ∈ [1,100]，但静音间隔段（frequency_hz=0）音量无意义；当前内置主题静音段填 1。建议 V1.1 明确：静音段 volume 可省略或允许 0~100（设备忽略静音段音量，V0.4 无限制）。
