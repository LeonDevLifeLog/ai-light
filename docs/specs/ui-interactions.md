...
- **默认进入快速创作**，不展示协议术语
- 标题栏右上角：[快速创作] [轨道工作台] 切换器
- 主题名输入 + 「正在编辑」预览始终可见
- 内置主题始终另存为用户主题，且不得与内置主题同名

### 7.2 快速创作

一屏只编辑一个状态（默认 `WORKING`），用户直接描述想看到的结果：
...
- 标准 5 态固定保留，并允许添加、删除自定义状态
- “相位 / 曲线 / 占空比”不出现在快速创作中，由语义预设编译生成
- `brightness = 0` 表示该灯全黑
- 右侧软件预览始终反映草稿中的真实灯位、颜色与亮度
- 设备已连接时显示「在设备上试听」，将未保存草稿交给 `preview_scene(content)` 并以 `RESTART_SCENE` 重播
- 修改自动保存为本机草稿；重新打开同一来源主题时恢复
- 「从其他主题借用一个效果」可把任意内置/用户主题的标准状态效果复制到当前状态，形成混搭主题

### 7.3 轨道工作台

为顶、中、底三条灯轨提供精确配置，但继续使用用户语言：

| 步骤 | 内容 | 控件 |
|---|---|---|
| 运动方式 | 常亮 / 呼吸 / 闪烁 / 渐亮 / 渐弱 | 编译为 curve |
| 颜色与亮度 | 最暗色 / 最亮色 / 0~100 | low / high / brightness |
| 动画节奏 | 一轮用时、出场时间 | period_ms / phase_deg |
| 三灯关系 | 一起 / 从上往下 / 从下往上 / 交错 | 相位组合预设 |
| 播放结束 | 重复次数 + 熄灭 / 保持暗色 / 保持亮色 | repeat / end_level |
| 状态切换 | 立即 / 快速柔化 / 自然 / 缓慢 | transition_ms |
| 声音 | 音调 / 停顿片段 + 重复 | buzzer segments |

界面主要标签不得使用“相位”；角度值只作为“出场时间”的精确值展示。

### 7.4 模式切换的副作用

```
[快速创作] → [轨道工作台]    展开精确字段
[轨道工作台] → [快速创作]    隐藏精确字段；数据保留
```
