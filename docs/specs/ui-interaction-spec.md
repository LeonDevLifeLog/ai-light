...
**6.5.1 用途**
Integrations 页的官方 Adapter 卡（Claude Code / Codex / WorkBuddy）。
...
**6.5.5 边界条件**
- Adapter 未安装或版本不兼容时，[连接] 先进入内联安装/升级确认态，用户确认后由后端经已解析工具链（Node + npm-cli.js）安装明确兼容版本；失败 Toast 展示脱敏 stderr 摘要。
