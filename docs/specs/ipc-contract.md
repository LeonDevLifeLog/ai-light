...
| `get_integration_status(tool)` | `claude-code \| codex \| qoder \| trae \| workbuddy` | Adapter 托管状态 | `BAD_REQUEST` / `ADAPTER_*` / `NODE_*` / `NPM_NOT_FOUND` / `TOOLCHAIN_*` | P1 |
| `install_integration(tool)` | `claude-code \| codex \| qoder \| trae \| workbuddy` | 写入结果 | `BAD_REQUEST` / `ADAPTER_*` / `NODE_*` / `NPM_NOT_FOUND` / `TOOLCHAIN_*` / `EXECUTABLE_TIMEOUT` | P1 |
| `uninstall_integration(tool)` | `claude-code \| codex \| qoder \| trae \| workbuddy` | 写入结果 | `BAD_REQUEST` / `ADAPTER_*` / `TOOLCHAIN_*` | P1 |
...
| `ADAPTER_NOT_FOUND` | Adapter CLI 不可执行 | 查询或管理 Claude Code/Codex/Qoder/TraeCode/WorkBuddy 接入 |
